...
if st.button("Analyze"):
    ...
    if result["success"]:
        ...
        with st.spinner("Reviewing..."):
            reviews = (run_review(files))
        df = (pd.DataFrame(reviews))
        st.session_state["df"] = df

if "df" in st.session_state:
    df = (st.session_state["df"])
    severity = (st.selectbox("Severity",["All", "Critical", "Major", "Minor", "High", "Medium", "Low"]))

    if severity != "All":
        df = (df[df["severity"] == severity])
    st.dataframe(df)
    markdown = (markdown_report(df))
    st.download_button("Download Markdown", markdown, "review.md")
    st.download_button("Download JSON", df.to_json(), "review.json")
    st.download_button("Download CSV", df.to_csv(), "review.csv")